#Script DAC scenario evaluation from the feasibility perspective
...
#genie_all <-vroom("C:/Users/brutschin/OneDrive - IIASA/DAC/Paper/data/published_data/gidden_brutshin_et_al_2023_data.csv")

data_path <- "C:/Users/brutschin/OneDrive - IIASA/DAC/Paper/data/raw_data/model" 
files <- dir(data_path, pattern = "*.xlsx") # get file names

genie_all <- files %>%
  map(~ read_xlsx(file.path(data_path, .))) %>% 
  reduce(rbind) 


#get the scenario list and short names

core<-read_xlsx("C:/Users/brutschin/OneDrive - IIASA/DAC/Paper/data/scenarios.xlsx", "core") 
governance<-read_xlsx("C:/Users/brutschin/OneDrive - IIASA/DAC/Paper/data/scenarios.xlsx", "governance") 

core_scenarios<-rbind(core, governance)

#import AR6 data

ar6<-vroom("C:/Users/brutschin/OneDrive - IIASA/DAC/Paper/data/ar6/AR6_Scenarios_Database_World_v1.0.csv") 
meta<-read_xlsx("C:/Users/brutschin/OneDrive - IIASA/DAC/Paper/data/ar6/Copy of AR6_Scenarios_Database_metadata_indicators_v1.1.xlsx", "meta") %>%
...
scenario_final<-scenario_set %>%
  left_join(core_scenarios)


scenario_final<-scenario_final %>%
  mutate(temperature2 = case_when( 
    Temperature == "1.5C" ~ "C1 - 1.5°C low overshoot", 
    Temperature == "1.5C - OS" ~ "C2 - 1.5°C high overshoot",
    Temperature == "2C" ~ "C3 - Below 2°C>67%")) %>%
  mutate(scenario=case_when(
    `Short Name`=="C1 without DAC"~"No DAC",
    `Short Name`=="C1 with DAC"~"DAC",
    `Short Name`=="C2 without DAC"~"No DAC",
    `Short Name`=="C2 with DAC"~"DAC",
    `Short Name`=="C2 with DAC and Governance SSP1"~"DAC governance",
    `Short Name`=="C3 without DAC"~"No DAC",
    `Short Name`=="C3 with DAC"~"DAC",
    `Short Name`=="C3 with DAC and Governance SSP2"~"DAC governance"))
...
scenarios_key<-c("C1 without DAC", "C1 with DAC","C2 without DAC", "C2 with DAC", 
                 ...
                 "C3 without DAC",
                 "C3 with DAC",
                 "C3 with DAC and Governance SSP2")

line_types<-c("No DAC"="solid","DAC"="dashed", "DAC governance"="dotted")
...
ar6_set<-ar6 %>%
  ...
  left_join(meta) %>%
  mutate(temperature2 = case_when( 
    Category == "C1" ~ "C1 - 1.5°C low overshoot", 
...
biomass1<-scenario_final %>%
  ...
  drop_na()
#%>%
#  filter(temperature2=="C1 - 1.5°C low overshoot")
...
biomass_trajectory<-ggplot()+
  ...
  geom_line(data=biomass2, aes(x=Year, y=primary_energy_biomass, group=Scenario_ID),color="grey", alpha=0.1)+
...
ccs_trajectory<-
  ...
  geom_line(data=ccs2, aes(x=Year, y=carbon_sequestration_ccs, group=Scenario_ID),color="grey", alpha=0.1)+
  ...
  annotate("rect", xmin = 2020, xmax = 2050, ymin = 4000, ymax = 8600,
           alpha = .3, fill="royalblue")+
  annotate("rect", xmin = 2020, xmax = 2050, ymin =8600, ymax = 30000,
           alpha = .3, fill="hotpink1")+
...
ggsave("global.jpeg", units="in", width=9, height=10, dpi=300)
...
solar_trajectory<-scenario_final %>%
  filter(Region!="World", Illustrative=="Yes", Year<2045) %>%
  filter(`Short Name` %in% scenarios_key) %>%
  ...
  drop_na() %>%
  mutate(scenario=factor(scenario, level=c("No DAC", "DAC", "DAC governance"))) %>%
...
coal_trajectory
ggsave("coal.jpeg", units="in", width=9, height=12, dpi=300)
...
wind_trajectory
ggsave("wind.jpeg", units="in", width=8, height=12, dpi=300)
