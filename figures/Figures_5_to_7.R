...
scenario_final<-scenario_set %>%
  ...
  mutate(scenario=case_when(
    ...
    `Short Name`=="C3 with Governance SSP2"~"Governance"))
...
line_types<-c("No DAC"="solid","DAC"="dashed", "DAC governance"="dotdash", "Governance"="dotted")
color_lines<-c("No DAC"="black","DAC"="brown4", "DAC governance"="blue", "Governance"="green4")
...
biomass1<-scenario_final %>%
 ...
 # filter(Region=="World", Illustrative=="Yes", Year<2055) %>%
  ...
  drop_na() %>%
  mutate(scenario=factor(scenario, levels=c("No DAC", "DAC", "Governance", "DAC governance")))
#%>%
...
biomass_trajectory<-ggplot()+
  ...
  #geom_line(data=biomass1, aes(x=Year, y=primary_energy_biomass, linetype=scenario, group=Scenario_ID, color=scenario), size=1.1)+
  geom_line(data=biomass1, aes(x=Year, y=primary_energy_biomass, linetype=scenario, group=Scenario_ID), size=1.1)+
...
solar_trajectory<-scenario_final %>%
  ...
  annotate("rect",xmin = -Inf, xmax = Inf, ymin = 20, ymax = 25,
           alpha = .3, fill="royalblue")+
  annotate("rect", xmin = -Inf, xmax = Inf,  ymin = 25, ymax = 30,
           alpha = .3, fill="hotpink1")+
  theme(legend.position="top")
...
wind_trajectory<-scenario_final %>%
  ...
  annotate("rect",xmin = -Inf, xmax = Inf, ymin = 10, ymax = 15,
           alpha = .3, fill="royalblue")+
  annotate("rect", xmin = -Inf, xmax = Inf,  ymin = 15, ymax = 30,
           alpha = .3, fill="hotpink1")+
  theme(legend.position="top")

wind_trajectory
ggsave("wind.jpeg", units="in", width=9, height=12, dpi=300)
